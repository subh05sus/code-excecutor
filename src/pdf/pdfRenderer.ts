import { PdfJobData, PdfResult, PdfRenderOptions } from "../types/pdf";
...
/**
 * Render self-contained HTML to a PDF buffer using the shared browser pool.
 * Blocks all network (only data:/about: allowed) so HTML must be fully inlined.
 */
  ...
  try {
    ...
    const pdfBuffer = await Promise.race([
      ...
      new Promise<never>((_, reject) =>
        setTimeout(() => reject(new Error("PDF render timed out")), pdfConfig.limits.renderTimeoutMs)
      ),
    ]);
...
export async function renderPdf(jobId: string, data: PdfJobData): Promise<PdfResult> {
  ...
  const { url, expiresAt } = await uploadAndSign(key, pdfBuffer, "application/pdf");
